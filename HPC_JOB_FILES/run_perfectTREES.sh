...
# Read  line from the list of tree paths
TREE_PATH=$(sed -n "${PBS_ARRAY_INDEX}p" plot_my_trees/all_tree_paths.txt)

# Run the Python script with the selected tree path
echo "Now processing: $TREE_PATH"
python3 plot_my_trees/get_perfectTREES.py "$TREE_PATH"

echo "finished making Z and and dendogram for $TREE_PATH"
